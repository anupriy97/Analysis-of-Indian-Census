...
#dataChildCrime = pd.read_csv("Data/Crime/Crime committed against children under different crime heads/chCAC.csv")
#dataEnrollment = pd.read_csv("Data/Crime/Major Education Statistics, 2004-05.csv")

#grpYearDataChildCrime = dataChildCrime.groupby("Year")
#avgGrpYearDataChildCrime = (grpYearDataChildCrime.get_group(2001).reset_index(drop=True).iloc[:, 2:]
 #					 + grpYearDataChildCrime.get_group(2002).reset_index(drop=True).iloc[:, 2:]
 #					 + grpYearDataChildCrime.get_group(2003).reset_index(drop=True).iloc[:, 2:]) / 3
